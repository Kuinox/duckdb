...
unique_ptr<ParquetColumnSchema> ParquetReader::ParseSchema(ClientContext &context) {
	...
	if (!file_meta_data->row_groups.empty() && next_file_idx != file_meta_data->row_groups[0].columns.size()) {
		throw IOException("Parquet file is malformed: schema declares %llu columns but row group only has %llu "
		                  "columns. The file may be corrupted or generated by a buggy writer.",
		                  next_file_idx, file_meta_data->row_groups[0].columns.size());
	}
