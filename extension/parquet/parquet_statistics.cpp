...
unique_ptr<BaseStatistics> ParquetStatisticsUtils::TransformColumnStatistics(const ParquetColumnSchema &schema,
	...
	// Check if the column index is within bounds of the row group columns
	if (schema.column_index >= columns.size()) {
		throw IOException("Parquet file is malformed: schema references column index %llu but row group only has %llu "
		                  "columns. The file may be corrupted or generated by a buggy writer.",
		                  schema.column_index, columns.size());
	}
